			...
			<!-- content starts -->
				<form action="addClass" method="post">
					<label>选择学年：</label><select id="selectYear" name="year"></select>
       				...
       				<label>选择课程：</label>
       				<select id="selectSubject" name="subject"></select>
       				<label>选择学院：</label>
       				<select id="selectDepartment" name="department" onchange="getClass('selectDepartment', 'selectClass')"></select>
       				<label>选择班级：</label>
				...
				<script type="text/javascript">
					//初始化学期信息
       				...
       				//初始化学院信息
       				$("selectDepartment").ready(function(){
       					var select_de = document.getElementById("selectDepartment");
       					$.post("queryDepartment",{},function(data, textStatus){
       						var dataArray = JSON.parse(data);
       						select_de.options.add(new Option("请选择", "void"));
       						for(var i=0;i<dataArray.length;i++){
       							select_de.options.add(new Option(dataArray[i].name, dataArray[i].id));
       						}
       					});
       				});
       				//Ajax刷新课程信息
       				...
       				function getClass(frist_type_id, next_sel_id) {
			            ...
			            next_sel.options.length = 0;
			            if (frist_type != "void") {
			                $.post('queryClass', { 'department':frist_type }, function (data, textStatus) {
			                    var dataArray = JSON.parse(data);
			                    next_sel.options.length = 0;
			                    next_sel.options.add(new Option("请选择", "void"));
			                    for (var i = 0; i < dataArray.length; i++) {
